"""
Train Tabular MLP for Outcome Model v2
...
model = OutcomeMLP(INPUT_DIM, hidden_dims=[512, 256, 128], dropout=0.4)
print(f"  Input dim: {INPUT_DIM}")
print(f"  Architecture: {INPUT_DIM} -> 512 -> 256 -> 128 -> 1")
...
for epoch in range(EPOCHS):
    ...
    if (epoch + 1) % 5 == 0 or epoch == 0:
        print(f"  Epoch {epoch+1:3d}: train_loss={train_loss:.4f}, val_loss={val_loss:.4f}, val_auc={val_auc:.4f}, val_acc={val_acc:.4f}")
    
    # Early stopping
    if val_auc > best_auc:
        best_auc = val_auc
        patience_counter = 0
        torch.save(model.state_dict(), OUTPUT_DIR / "model_outcome_mlp_best.pt")
    else:
        patience_counter += 1
...
# Load best model
model.load_state_dict(torch.load(OUTPUT_DIR / "model_outcome_mlp_best.pt"))
...
with open(OUTPUT_DIR / "results_mlp.json", 'w') as f:
...
print("="*60)
print(f"\nSaved:")
print(f"  - {OUTPUT_DIR / 'model_outcome_mlp_best.pt'}")
print(f"  - {OUTPUT_DIR / 'results_mlp.json'}")
