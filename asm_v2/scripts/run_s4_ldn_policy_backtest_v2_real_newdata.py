#!/usr/bin/env python3
"""
S4_LDN Policy Backtest v2 for GC M1 NEW DATA (OOS Phase 3)

Uses frozen policies - NO retraining.
...
def main():
    print("=" * 100)
    print("S4_LDN Policy Backtest v2 for GC M1 NEW DATA (OOS Phase 3)")
    print("=" * 100)
    
    config_path = "asm_v2/configs/s4_policy_config_gc_m1_real_newdata_v1.json"
    ...
    try:
        ...
        print("Please run run_s4_ldn_enrich_gc_m1_real_newdata.py first")
        sys.exit(1)
    ...
    # Evaluate
    print(f"\nEvaluating policies (NO ML meta-model for OOS)...")
    bt.evaluate_policies(
        policies=policies,
        include_ml_policies=False,  # No ML for OOS
        ml_thresholds=[],
    )
    ...
    league_json_path = f"{output_dir}/{output_files.get('league_json', 's4_policy_league_gc_m1_real_newdata_v1.json')}"
    with open(league_json_path, 'w') as f:
        json.dump(results_data, f, indent=2)
    
    league_csv_path = f"{output_dir}/{output_files.get('league_csv', 's4_policy_league_gc_m1_real_newdata_v1.csv')}"
    ...
    if best:
        ...
        best_path = f"{output_dir}/{output_files.get('best_policy', 's4_policy_best_gc_m1_real_newdata_v1.json')}"
        with open(best_path, 'w') as f:
            json.dump(best_data, f, indent=2)
    
    print(f"\n" + "=" * 100)
    print(f"Results saved to: {output_dir}/")
    print(f"  - {output_files.get('league_json')}")
    print(f"  - {output_files.get('league_csv')}")
    print(f"  - {output_files.get('best_policy')}")
    print("=" * 100)
